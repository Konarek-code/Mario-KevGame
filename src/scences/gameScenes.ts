...
export default class GameScene extends Phaser.Scene {
  ...
  private bags!: Phaser.Physics.Arcade.StaticGroup;
  private enemies!: Phaser.Physics.Arcade.Group;
  private waterZones!: Phaser.Physics.Arcade.StaticGroup;
  private abyssZones!: Phaser.Physics.Arcade.StaticGroup;
  private finishZone!: Phaser.Physics.Arcade.StaticGroup;
  private selectedCharacterKey: string = "player1";
  ...
  init(data: { selectedCharacter?: string; levelKey?: string }) {
  ...
  create() {
    ...
    const heroToPlayerMap: Record<string, string> = {
      hero1: "player1",
      hero2: "player2",
      hero3: "player3",
    };
    ...
    (this.player.body as Phaser.Physics.Arcade.Body).setSize(32, 60);
    ...
    if (obstacleLayer && obstacleLayer.objects) {
      obstacleLayer.objects.forEach((obj) => {
        ...
        if (obj.type === "shooters") {
          ...
          const delay =
            obj.properties?.find(
              (p: { name: string; value: any }) => p.name === "delay"
            )?.value || 3000;
  ...
  update() {
    const speed = 160;
    const jumpSpeed = -400;
    const onGround = (this.player.body as Phaser.Physics.Arcade.Body).blocked
      .down;

    if (this.cursors.left?.isDown || this.moveLeft) {
      this.player.setVelocityX(-speed);
      if (this.hasAnimations) this.player.anims.play("run", true);
      ...
      if (this.hasAnimations) this.player.anims.play("idle", true);
    }

    if (
      (Phaser.Input.Keyboard.JustDown(this.cursors.up!) || this.jump) &&
      onGround
    ) {
    ...
    this.movingPlatforms.children.iterate(
  ...
  private fireBullet(x: number, y: number, direction: string) {
